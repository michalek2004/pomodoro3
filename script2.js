...
function startTimer() {
    timer = setInterval(() => {
        if (remainingTime > 0) {
            ...
            emailFromLS = localStorage.getItem('email')
            console.log(341)
            fetch("http://127.0.0.1:5000/updatePomodoro", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
...
function updateCalendar() {
    ...
    fetch(`http://127.0.0.1:5000/getStatistics?email=${encodeURIComponent(emailFromLS2)}&YearAndMonth=${encodeURIComponent(yearMonthToBackEnd)}`, {
        method: 'GET'
    })
    .then(response => response.json())
    .then(data => {
